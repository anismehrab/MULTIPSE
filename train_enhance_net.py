...
from data.enhance_transform import DataBatch, ToTensor,Normalize
from torch.utils.data import DataLoader
from models import enhance_model
from models.rident import ridnet
from utils.train_utils import train,valid,save_checkpoint,test_with_image
...
parser.add_argument("--checkpoint_path", type=str, default="checkpoints/enhance_net_checkpoints/enhance_net_x1/v5",help="checkpoint_folder_path")
parser.add_argument("--logger_path", type=str, default="checkpoints/enhance_net_checkpoints/enhance_net_x1/v5/train_logging.log",help="logger path")
...
parser.add_argument("--max_cells", type=int, default=360*340,help="min image dimension")#x2 250*250 b8 #x3 190*185 b4 #x1 500*500
...
def reInitLoader(box):
    ...
    dataBatch = DataBatch(transfrom=batch_compos,scale =args.scale ,max_box = box,
                max_cells= args.max_cells,devider=8,force_size=None,use_whole_image= True,workers = args.threads)
...
#load models
print("loading model")
model =  enhance_model.EnhanceNetX1_v5()#ridnet.RIDNET(nf=64,img_range=1.)
...
for i in range(epoch_i,epoch_i+args.epoch):
    ...
    test_with_image(model[0],i,device)
