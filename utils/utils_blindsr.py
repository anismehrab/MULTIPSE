...
def random_crop(lq, hq, sf=4, lq_patchsize_w=64,lq_patchsize_h=64):
    h, w = lq.shape[:2]
...
def degradation_bsrgan_plus(img, sf=4, shuffle_prob=0.5, use_sharp=False, lq_patchsize_w=64,lq_patchsize_h=None, isp_model=None):
    """
    ...
    # random crop
    img, hq = random_crop(img, hq, sf, lq_patchsize_w,lq_patchsize_h)

    return img, hq


def degradation_bsrgan_plus_an(img,hq=None, sf=4, shuffle_prob=0.5, use_sharp=False, lq_patchsize_w=64,lq_patchsize_h=64, isp_model=None,degrade =True ,noise = True):
    """
    This is an extended degradation model by combining
    the degradation models of BSRGAN and Real-ESRGAN
    ----------
...
if __name__ == '__main__':
    in_img_path = "/media/anis/InWork/Data/dataset/DIV_FLICKR_2K/valid/"
    img = util.imread_uint(in_img_path, 3)
    img = util.uint2single(img)
    sf = 4
    import time
    out_deg1_path = "x"+str(sf)+"_"
    for i in range(20):
        start = time.time()
        img_lq, img_hq = degradation_bsrgan(img, sf=sf, lq_patchsize=240)
        ...
        util.imsave(img_concat, out_deg1_path+str(i)+'.png')


    # out_deg1_path = "x"+str(sf)+"_"
    # for i in range(20):
    #     img_lq, img_hq = degradation_bsrgan_plus(img, sf=sf, shuffle_prob=0.1, use_sharp=True, lq_patchsize=240)
    #     print(i)
    #     lq_nearest =  cv2.resize(util.single2uint(img_lq), (int(sf*img_lq.shape[1]), int(sf*img_lq.shape[0])), interpolation=0)
    #     img_concat = np.concatenate([lq_nearest, util.single2uint(img_hq)], axis=1)
    #     util.imsave(img_concat, out_deg1_path+str(i)+'.png')
