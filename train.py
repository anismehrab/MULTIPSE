...
parser.add_argument("--checkpoint_path", type=str, default="checkpoints",help="checkpoint_folder_path")
parser.add_argument("--logger_path", type=str, default="train_logging.log",help="logger path")
...
def reInitLoader(box):
    ...
    dataBatch = DataBatch(transfrom=batch_compos,scale =args.scale ,max_box = box,max_cells= args.max_cells)
    training_data = Dataset(data_dir=args.data_train,aug_num=0,transform=data_compos)
    validation_data = Dataset(data_dir=args.data_valid,aug_num=0,transform=data_compos)
...
#optimzer
params = list(base_model.parameters()) + list(head_model.parameters())
