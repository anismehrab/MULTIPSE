...
class AnimeNet5(nn.Module):
    def __init__(self, in_nc=3, nf=64,out_nc=3,act_type = 'relu'):
        ...
        self.IMDB15 = B.IMDModule(in_channels=nf,cc_acti=act_type,act_type=act_type)
        self.IMDB16 = B.IMDModule(in_channels=nf,cc_acti=act_type,act_type=act_type)
        num_modules=16
    ...
    def forward(self, input):
        ...
        # print(input[0].size())
        # print(input[0].size())
        in_O = self.downsampleer_O(input[0])
        in_A  = self.downsampleer_A(input[1])
        in_ = torch.add(in_O,in_A)
        out_B1 = self.IMDB1(in_)
        out_B2 = self.IMDB2(out_B1)
        out_B3 = self.IMDB3(out_B2)
        out_B4 = self.IMDB4(out_B3)
        out_B5 = self.IMDB5(out_B4)
        ...
        out_B13 = self.IMDB13(out_B12)
        out_B14 = self.IMDB14(out_B13)       
        out_B15 = self.IMDB15(out_B14)
        out_B16 = self.IMDB16(out_B15)    
        out_B = self.conv_cat(torch.cat([out_B1, out_B2, out_B3, out_B4, out_B5, out_B6, out_B7, out_B8, out_B9, out_B10, out_B11, out_B12, out_B13, out_B14,out_B15,out_B16], dim=1))
         


        out_lr = torch.add(self.LR_conv(out_B), in_)
